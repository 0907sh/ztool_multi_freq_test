...
def frequency_sweep(t_snap=None, t_sim=None, t_step=None, sample_step=None, v_perturb_mag=None,freq=None, f_points=None,
    ...
    for identification in scanid:
        ...
        print(f' [DEBUG] find_all(Name="{identification}") → {len(blocks)} block(s) found')
        for b in blocks:
            print(f'         defn_name={b.defn_name!r}  joined="{" ".join(str(x) for x in b.defn_name)}"')
        # blocks_tool = [block for block in blocks if "Z_tool" in block.defn_name[0]]  # Filter only Z-tool components
        blocks_tool = [block for block in blocks if "Z_tool" in "".join(str(x) for x in block.defn_name)]  # Filter only Z-tool components
        if not blocks_tool:
            # Fallback: defn_name 형식이 다른 경우 (라이브러리명 소문자 등)
            blocks_tool = [block for block in blocks if "tool" in "".join(str(x) for x in block.defn_name).lower()]
        if not blocks_tool:
            # 최후 수단: find_all 결과를 그대로 사용 (이미 find_first로 Z_tool 블럭임을 확인함)
            blocks_tool = blocks
        if not blocks_tool:
            raise RuntimeError(f'No scan block found with Name="{identification}". Check that the Z_tool PCC block exists on the Main canvas.')
        if "DC" in "".join(str(x) for x in blocks_tool[0].defn_name):
